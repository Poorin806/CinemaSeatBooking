...
public class SeatBookingGUI {
    ...
    private static ArrayList<String> seatId = new ArrayList<String>();
    private static ArrayList<JToggleButton> seatButtons = new ArrayList<>();
    private static ArrayList<String> seatBooking = new ArrayList<>();
    ...
    public static JPanel get() {
        ...
        // Seat Panel
        JPanel seatPanel = new JPanel(new FlowLayout(FlowLayout.CENTER, 5, 5)); // ใช้ GridLayout
        ...
        for (int i = 0; i < 76; i++) {
            ...
            // Header Rows (แสดงหัวแถวทุกๆ 15 ปุ่ม)
            if (i % 14 == 0) {
                ...
                rows = 0;  // รีเซ็ต rows ทุกแถวใหม่
                currentHeader++;
            }

            JToggleButton seatBtn = new JToggleButton();
            seatBtn.setText(Integer.toString(rows + 1));  // แสดงหมายเลขที่นั่ง (เริ่มจาก 1)
            ...
            seatBtn.addItemListener(e -> {
                if (e.getStateChange() == ItemEvent.SELECTED) {
                    System.out.println("Seat Booking: " + seatId.get(finalI));
                    seatBooking.add(seatId.get(finalI));
                } else if (e.getStateChange() == ItemEvent.DESELECTED) {
                    System.out.println("Cancel Booking: " + seatId.get(finalI));
                    seatBooking.remove(seatId.get(finalI));
                }
            });
    ...
    private static void cartSeatBooking() {
       ...
       if (seatBooking.isEmpty()) System.out.println("- You are have no booking yet...");
       else for (String s : seatBooking) System.out.print(s + ", ");
    }
